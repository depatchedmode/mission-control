/**
 * Automerge-based Gameplan Store
 * 
...
export class AutomergeStore {
  constructor(options = {}) {
    ...
    const envStoragePath = env.GP_STORAGE_PATH
    this.storagePath = options.storagePath ||
      (envStoragePath ? join(envStoragePath, '.gameplan') : join(process.cwd(), '.gameplan'))
    this.urlFile = options.urlFile ??
      (options.storagePath || envStoragePath
        ? join(this.storagePath, 'document-url')
        : join(process.cwd(), '.gameplan-url'))
    this.usePersistedUrl = options.usePersistedUrl ?? Boolean(this.urlFile)
    this.logger = options.logger ?? console
    this.mentionClaimTtlMs = Number(
      options.mentionClaimTtlMs ?? env.GP_MENTION_CLAIM_TTL_MS ?? 30000
    )

    if (!Number.isInteger(this.mentionClaimTtlMs) || this.mentionClaimTtlMs <= 0) {
      throw new Error(`Invalid GP_MENTION_CLAIM_TTL_MS: ${env.GP_MENTION_CLAIM_TTL_MS}`)
    }
  ...
  async init() {
    ...
    try {
      ...
      if (this.usePersistedUrl && existsSync(this.urlFile)) {
        ...
        this.logger.log?.('📂 Loading existing Gameplan document:', existingUrl)
        ...
        // Create new document
        this.logger.log?.('🆕 Creating new Gameplan document...')
