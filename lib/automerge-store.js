...
export class AutomergeStore {
  constructor(options = {}) {
    const storagePath = options.storagePath || join(process.cwd(), '.mission-control')
    const urlFile = options.urlFile || join(process.cwd(), '.mission-control-url')

    // For isolated test stores, default to creating a fresh document.
    this.usePersistedUrl = options.usePersistedUrl ?? !options.storagePath
