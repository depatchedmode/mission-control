...
export class AutomergeStore {
  constructor(options = {}) {
    this.storagePath = options.storagePath || join(process.cwd(), '.mission-control')
    this.urlFile = options.urlFile ??
      (options.storagePath ? join(this.storagePath, 'document-url') : join(process.cwd(), '.mission-control-url'))
    ...
    try {
      ...
      if (this.usePersistedUrl && existsSync(this.urlFile)) {
        ...
        if (this.usePersistedUrl) {
          ...
          console.log(`💾 Document URL saved to ${this.urlFile}`)
        }
      }
  ...
  async getPendingMentions(agent = null) {
    ...
    let mentions = Object.values(doc.mentions || {})
      ...
      .map(mention => normalizeTaskScopedRecord(mention))
  ...
  getDoc() {
    ...
    doc.mentions = normalizeTaskScopedRecordMap(doc.mentions || {})
    return doc
  }
