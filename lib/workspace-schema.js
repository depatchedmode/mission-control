...
export function taskAlternatives(doc, taskId, field) {
  taskRecord(doc, taskId)
  return alternatives(doc, doc.taskFields, taskFieldKey(taskId, field))
}

export const taskFieldKey = (taskId, field) => `${taskId}:${field}`
...
export function taskState(doc, taskId) {
  ...
  for (const field of TASK_FIELDS) {
    ...
    const selected = selectedAlternative(choices, doc.taskFields[taskFieldKey(taskId, field)])
  ...
  return {
    task: { id: taskId, ...fields, created_at: task.created_at, updated_at: updated || task.created_at, created_by: task.actorId,
      ...(task.branch ? { branch_of: task.branch.parentId, branch_name: task.branch.name, merged: Boolean(doc.mergedBranches[taskId]) } : {}) },
    revisions, conflicts,
  }
}
