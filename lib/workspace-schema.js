...
export function createWorkspaceData({ actors = [], workspaceId = randomUUID(), now = new Date().toISOString() } = {}) {
  ...
  requireValue(new Set(records.map(actor => actor.id)).size === records.length, 'Actor IDs must be unique')
  requireValue(new Set(records.map(actor => actor.handle)).size === records.length, 'Actor handles must be unique')
  return {
...
export function resolveActor(doc, reference) {
  ...
  const actor = Object.hasOwn(doc.actors, reference) ? doc.actors[reference]
    : Object.values(doc.actors).find(candidate => candidate.handle === reference.toLowerCase())
