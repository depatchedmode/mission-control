import {
  TASK_FIELDS, OperationError, actorRecord, alternatives, canonical, checkRevisions,
  digest, identifier, normalizeFields, object, plain, receiptKey, requireValue,
  resolveActor, taskContext, taskRecord, taskView, text,
...
function newTask(doc, fields, event, taskId) {
  ...
  return {
    id: taskId, actorId: event.actorId, created_at: event.timestamp,
...
function fieldChanges(doc, taskId, updates, expected, allowConflict = false) {
  ...
  return Object.entries(updates).map(([field, value]) => {
    const revisions = alternatives(task.fields, field).map(record => record.operationId)
    checkRevisions(revisions, expected?.[field], allowConflict)
    return { field, old: plain(task.fields[field].value), new: value }
  })
}

function writeFields(draft, taskId, changes, event) {
  for (const change of changes) draft.tasks[taskId].fields[change.field] = register(change.new, event)
}
...
function writeComment(draft, taskId, parts, event) {
  draft.comments[parts.commentId] = {
    ...
    body: parts.body, deleted: register(false, event),
  }
...
function existingComment(doc, commentId) {
  ...
  requireValue(comment && !comment.deleted.value, `Comment not found: ${commentId}`, 'NOT_FOUND')
  return comment
}

function prepareActor(doc, payload, event) {
  const actor = actorRecord(payload)
  requireValue(!Object.hasOwn(doc.actors, actor.id), 'Actor ID already exists', 'ALREADY_EXISTS')
  requireValue(!Object.values(doc.actors).some(value => value.handle === actor.handle), 'Actor handle already exists', 'ALREADY_EXISTS')
...
function prepareCreate(doc, payload, event) {
  ...
  return { taskId, result: { taskId }, changes: Object.entries(task.fields).map(([field, record]) => ({ field, old: null, new: record.value })),
    change: draft => { draft.tasks[taskId] = task } }
}
...
function prepareCommentEdit(doc, payload, event) {
  ...
  checkRevisions(alternatives(comment, 'body').map(record => record.operationId), payload.expectedRevisions, event.type === 'comment.resolve')
  ...
  return { taskId: comment.taskId, result: { taskId: comment.taskId, commentId: comment.id },
    change: draft => {
      draft.comments[comment.id].body = parts.body
      writeMentions(draft, parts.mentions)
    } }
}
...
function prepareCommentDelete(doc, payload, event) {
  const comment = existingComment(doc, payload.commentId)
  checkRevisions(alternatives(comment, 'body').map(record => record.operationId), payload.expectedRevisions)
  return { taskId: comment.taskId, result: { taskId: comment.taskId, commentId: comment.id },
    change: draft => { draft.comments[comment.id].deleted = register(true, event) } }
}
...
function prepareBranch(doc, payload, event) {
  ...
  task.branch = { parentId: payload.taskId, name: payload.name, base, merged: register(false, event) }
...
function prepareMerge(doc, payload, event) {
  ...
  requireValue(branch.branch && !branch.branch.merged.value, 'Choose an unmerged branch', 'INVALID_BRANCH')
  const parentId = branch.branch.parentId
  const parent = taskView(doc, parentId)
  const values = taskContext(doc, payload.branchId)
  requireValue(Object.keys(values.conflicts).length === 0, 'Resolve branch conflicts before merging', 'CONFLICT_REQUIRES_RESOLUTION')
  const updates = {}
  for (const field of TASK_FIELDS) {
    if (field === 'order' || canonical(values.task[field]) === canonical(branch.branch.base[field])) continue
    requireValue(canonical(parent[field]) === canonical(branch.branch.base[field]) || canonical(parent[field]) === canonical(values.task[field]),
      `Both the parent and branch changed ${field}; reconcile them before merging`, 'BRANCH_CONFLICT', { field, parent: parent[field], branch: values.task[field] })
    updates[field] = values.task[field]
  }
  ...
  return { taskId: parentId, result: { taskId: parentId, parentId, branchId: branch.id }, changes,
    change: draft => {
      ...
      draft.tasks[branch.id].branch.merged = register(true, event)
      draft.tasks[branch.id].fields.status = register('completed', event)
    } }
}
...
export function prepareOperation(doc, request, { replicaId, timestamp }) {
  ...
  event.changes = prepared.changes ?? []
