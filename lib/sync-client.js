export class SyncRequestError extends Error {
  constructor(message, options = {}) {
    ...
    this.code = options.code ?? (this.isTransport ? 'LOCAL_SERVICE_UNAVAILABLE' : 'HTTP_ERROR')
...
export async function requestJson(baseUrl, path, options = {}) {
  ...
  if (!res.ok) {
    const payload = res.headers.get('content-type')?.includes('application/json') ? await res.clone().json().catch(() => null) : null
