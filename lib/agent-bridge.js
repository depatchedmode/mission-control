...
export class AgentBridge {
  ...
  async dispatch(mapping) {
    ...
    resolveActor(doc, row.mention.fromActorId)
    const context = await this.source.context(row.mention)
    if (!context.mentions.some(mention => canonical(mention) === canonical(row.mention))
      || !context.comments.some(comment => comment.id === row.mention.commentId)) {
      return deny('waiting: originating context has not arrived or the mention was withdrawn')
    }
